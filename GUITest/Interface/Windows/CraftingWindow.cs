  ...
  public class CraftingWindow : Window
  {
    ...
    public CraftingWindow(ContentManager content) : base(content)
    {
      ...
      _itemSection = new WindowSection()
      {
        ...
        Items = new List<Button>()
        {
          new Button(content.Load<Texture2D>("Interface/ToolbarIcons/Map")),
          new Button(content.Load<Texture2D>("Interface/ToolbarIcons/Squad")),
          new Button(content.Load<Texture2D>("Interface/ToolbarIcons/Squad")),
          new Button(content.Load<Texture2D>("Interface/ToolbarIcons/Map")),
          new Button(content.Load<Texture2D>("Interface/ToolbarIcons/Squad")),
          new Button(content.Load<Texture2D>("Interface/ToolbarIcons/Squad")),
      ...
      var x = 10 + (buttonTexture.Width / 2);
      var y = 30;
      ...
      {
        _categorySection.Items.Add(new Button(buttonTexture, buttonFont)
        {
          Text = category,
          Position = new Vector2(x, y)
        });

        y += buttonTexture.Height + 5;
      }
    ...
    public override void SetPositions()
    {
      ...
      Position = new Vector2((Game1.ScreenWidth / 2) - (Texture.Width / 2), screenHeight - Texture.Height - (buttonHeight * 2));
      ...
      foreach (var button in _itemSection.Items)
      {
        switch (button.CurrentState)
        {
          ...
          case ToolbarButtonStates.Clicked:
            ...
            if (clicked && (mouseRectangleWithCamera_Items.Intersects(windowRectangle)) && !_itemSection.Items.Any(c => c != button && c.Rectangle.Intersects(mouseRectangleWithCamera_Items))) // Check if we're clicking somewhere that isn't on any button
            {
              foreach (var b in _itemSection.Items)
                b.CurrentState = ToolbarButtonStates.Nothing;
      ...
      foreach (var button in _categorySection.Items)
      {
        switch (button.CurrentState)
        {
          ...
          case ToolbarButtonStates.Clicked:
            ...
            if (clicked && (mouseRectangleWithCamera_Categories.Intersects(windowRectangle)) && !_itemSection.Items.Any(c => c != button && c.Rectangle.Intersects(mouseRectangleWithCamera_Categories))) // Check if we're clicking somewhere that isn't on any button
            {
              foreach (var b in _itemSection.Items)
                b.CurrentState = ToolbarButtonStates.Nothing;
